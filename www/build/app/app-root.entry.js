...
import { d as rIC, e as assert, f as debounce } from './chunk-5a5b5816.js';
...
function importTapClick(win, config) {
    import('./chunk-24d9cfab.js').then(module => module.startTapClick(win.document, config));
}
function importInputShims(win, config) {
    const inputShims = config.getBoolean("inputShims", needInputShims(win));
    if (inputShims) {
        import('./chunk-624b6e0f.js').then(module => module.startInputShims(win.document, config));
    }
}
